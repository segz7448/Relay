...
function TabChip({ focused, config, badge, onPress, colors, styles }) {
  ...
  const onPressIn = () => Animated.spring(scale, { toValue: 0.9, useNativeDriver: true, speed: 40, bounciness: 6 }).start();
  ...
  return (
    <View style={[styles.wrap, { bottom: Math.max(insets.bottom, space.md) }]} pointerEvents="box-none">
      <View style={styles.barClip}>
        <BlurView intensity={45} tint={scheme === 'light' ? 'light' : 'dark'} style={StyleSheet.absoluteFill} />
...
function getStyles(colors, scheme) {
  ...
  return StyleSheet.create({
    wrap: { position: 'absolute', left: space.lg, right: space.lg, alignItems: 'stretch' },
    barClip: {
    ...
    tint: {
      ...
      backgroundColor: isLight ? 'rgba(255,255,255,0.55)' : 'rgba(20,24,28,0.45)',
    },
    row: { flexDirection: 'row', paddingVertical: space.sm, paddingHorizontal: space.xs },
    ...
    chip: {
      ...
      backgroundColor: isLight ? 'rgba(20,24,28,0.04)' : 'rgba(255,255,255,0.05)',
      borderWidth: 1,
      borderColor: isLight ? 'rgba(20,24,28,0.08)' : 'rgba(255,255,255,0.08)',
    },
    chipActive: {
