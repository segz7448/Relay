...
function ActionButton({ icon, label, color, onPress, styles }) {
  return (
    <Pressable onPress={onPress} style={[styles.actionBtn, { backgroundColor: color }]}>
...
export default function ConversationRow({ convo, onPress, onLongPress, onPin, onMarkRead, onMute, onDelete, selectable, selected, onToggleSelect }) {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, g) => {
        const next = Math.max(-RIGHT_MAX, Math.min(LEFT_MAX, offset.current + g.dx));
  ...
  const showOnlineDot = convo.online && (convo.kind === 'direct' || convo.kind === 'bot');
  const badgeColor = convo.muted ? colors.textMuted : colors.accent;

  return (
    <View style={styles.wrap}>
      <View style={[styles.actionsRow, { left: 0 }]}>
        <ActionButton icon="checkmark-done" label={convo.unread > 0 ? 'Read' : 'Unread'} color={colors.online} onPress={() => fire(onMarkRead)} styles={styles} />
        <ActionButton icon="pin" label={convo.pinned ? 'Unpin' : 'Pin'} color={colors.accent} onPress={() => fire(onPin)} styles={styles} />
      </View>
      <View style={[styles.actionsRow, { right: 0 }]}>
        <ActionButton icon="volume-mute" label={convo.muted ? 'Unmute' : 'Mute'} color={colors.textSecondary} onPress={() => fire(onMute)} styles={styles} />
        <ActionButton icon="trash" label="Delete" color={colors.danger} onPress={() => fire(onDelete)} styles={styles} />
      </View>

      <Animated.View style={{ transform: [{ translateX }], backgroundColor: colors.bg }} {...(selectable ? {} : panResponder.panHandlers)}>
        <Pressable
          onPress={() => (selectable ? onToggleSelect?.(convo) : onPress?.(convo))}
          ...
          {selectable ? (
            <View style={styles.checkboxSlot}>
              <View style={[styles.checkbox, selected && styles.checkboxOn]}>
                {selected ? <Ionicons name="checkmark" size={14} color="#14181C" /> : null}
              </View>
            </View>
          ) : (
            <View style={styles.avatarWrap}>
              <View style={[styles.avatar, { backgroundColor: hashColor(convo.id) }]}>
                <Text style={styles.avatarText}>{initials(convo.name)}</Text>
              </View>
          ...
          <View style={styles.main}>
            <View style={styles.topLine}>
              <View style={styles.nameRow}>
                ...
                {convo.muted ? <Ionicons name="volume-mute" size={13} color={colors.textMuted} style={{ marginLeft: 5 }} /> : null}
              </View>
            ...
            <View style={styles.bottomLine}>
              <Text
                style={[styles.subtitle, convo.typing && styles.typing, convo.unread > 0 && !convo.typing && styles.subtitleUnread]}
                numberOfLines={1}
              >
                {subtitle}
              </Text>
              <View style={styles.trailing}>
                {convo.pinned ? <Ionicons name="pin" size={13} color={colors.textMuted} style={{ marginRight: convo.unread > 0 ? 6 : 0 }} /> : null}
                {convo.unread > 0 ? (
                  <View style={[styles.badge, { backgroundColor: badgeColor }]}>
                    <Text style={styles.badgeText}>{convo.unread > 99 ? '99+' : convo.unread}</Text>
                  </View>
                ) : null}
              </View>
            </View>
...
function getStyles(colors) {
  ...
  name: { ...type.h2, fontSize: 16, color: colors.textPrimary, flexShrink: 1 },
  time: { ...type.small, color: colors.textMuted, marginLeft: space.sm },
  bottomLine: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 3 },
  subtitle: { ...type.body, color: colors.textMuted, flex: 1, marginRight: space.sm },
  ...
  badgeText: { ...type.small, color: '#14181C', fontWeight: '700', fontSize: 11 },
  separator: {
