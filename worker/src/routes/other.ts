...
import { uid, generateApiKey, generateWebhookSecret, hashToken } from '../lib/keys';
...
// ─── Privacy ──────────────────────────────────────────────────────────────────
export const privacy = new Hono<{ Bindings: any; Variables: { user: UserSession } }>();
...
privacy.get('/blocked', async (c) => {
  ...
  return c.json((rows.results ?? []).map((b: any) => ({
    id: b.blocked_id, name: b.blocked_name, username: b.blocked_username, blockedAt: b.blocked_at,
  })));
});

privacy.post('/blocked', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const { id, name, username } = await c.req.json<{ id: string; name: string; username: string }>();
  const rid = uid();
  ...
  ).bind(rid, userId, id, name, username, Date.now()).run();
  return c.json({ ok: true }, 201);
});

privacy.delete('/blocked/:blockedId', async (c) => {
  const { userId } = c.get('user') as UserSession;
  await (c.env.DB as D1Database).prepare('DELETE FROM blocked_users WHERE blocker_id = ? AND blocked_id = ?')
    .bind(userId, c.req.param('blockedId')).run();
  return c.json({ ok: true });
});

privacy.get('/settings', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const row = await (c.env.DB as D1Database).prepare('SELECT * FROM privacy_settings WHERE user_id = ?').bind(userId).first<any>();
  if (!row) return c.json({ error: 'not_found' }, 404);
  return c.json({
    phoneNumber: row.phone_number, lastSeen: row.last_seen, profilePhoto: row.profile_photo,
    calls: row.calls, forwardedMessages: row.forwarded_msgs, groups: row.groups,
    voiceMessages: row.voice_messages,
    alerts: {
      newLogin: !!row.alert_new_login, newDevice: !!row.alert_new_device,
      failedAttempts: !!row.alert_failed_attempts, apiKeyUsage: !!row.alert_api_key_usage,
      emailAlerts: !!row.alert_email, pushAlerts: !!row.alert_push,
    },
  });
});
...
privacy.patch('/settings', async (c) => {
  ...
  const colMap: Record<string, string> = {
    phoneNumber: 'phone_number', lastSeen: 'last_seen', profilePhoto: 'profile_photo',
    calls: 'calls', forwardedMessages: 'forwarded_msgs', groups: 'groups', voiceMessages: 'voice_messages',
  };
  ...
  for (const [k, v] of Object.entries(body)) {
    ...
    if (k === 'alerts' && typeof v === 'object' && v) {
      const a = v as any;
      if (a.newLogin !== undefined) { updates.push('alert_new_login = ?'); vals.push(a.newLogin ? 1 : 0); }
      if (a.newDevice !== undefined) { updates.push('alert_new_device = ?'); vals.push(a.newDevice ? 1 : 0); }
      if (a.failedAttempts !== undefined) { updates.push('alert_failed_attempts = ?'); vals.push(a.failedAttempts ? 1 : 0); }
      if (a.apiKeyUsage !== undefined) { updates.push('alert_api_key_usage = ?'); vals.push(a.apiKeyUsage ? 1 : 0); }
      if (a.emailAlerts !== undefined) { updates.push('alert_email = ?'); vals.push(a.emailAlerts ? 1 : 0); }
      if (a.pushAlerts !== undefined) { updates.push('alert_push = ?'); vals.push(a.pushAlerts ? 1 : 0); }
    }
  }
  if (!updates.length) return c.json({ error: 'nothing_to_update' }, 400);
  vals.push(userId);
  await (c.env.DB as D1Database).prepare(`UPDATE privacy_settings SET ${updates.join(', ')} WHERE user_id = ?`).bind(...vals).run();
  return c.json({ ok: true });
});

// ─── Developer Platform (API Keys + Webhooks) ─────────────────────────────────
export const dev = new Hono<{ Bindings: any; Variables: { user: UserSession } }>();
dev.use('*', requireAuth);
...
dev.post('/api-keys', async (c) => {
  ...
  ).bind(id, userId, name, hash, prefix, scope ?? 'Full access', Date.now()).run();
  return c.json({ id, name, key: raw, prefix, scope: scope ?? 'Full access' }, 201);
});

dev.delete('/api-keys/:keyId', async (c) => {
  const { userId } = c.get('user') as UserSession;
  await (c.env.DB as D1Database).prepare('UPDATE api_keys SET revoked = 1 WHERE id = ? AND user_id = ?')
    .bind(c.req.param('keyId'), userId).run();
  return c.json({ ok: true });
});
...
dev.post('/webhooks', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const body = await c.req.json<{ description?: string; url: string; events?: string[] }>();
  ...
  ).bind(id, userId, body.description ?? '', body.url, secretHash, secretPrefix, JSON.stringify(body.events ?? []), Date.now()).run();
...
dev.patch('/webhooks/:whId', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const body = await c.req.json<{ description?: string; url?: string; events?: string[]; enabled?: boolean }>();
  const updates: string[] = [];
  const vals: unknown[] = [];
  if (body.description !== undefined) { updates.push('description = ?'); vals.push(body.description); }
  if (body.url !== undefined) { updates.push('url = ?'); vals.push(body.url); }
  if (body.events !== undefined) { updates.push('events = ?'); vals.push(JSON.stringify(body.events)); }
  if (body.enabled !== undefined) { updates.push('enabled = ?'); vals.push(body.enabled ? 1 : 0); }
  if (!updates.length) return c.json({ error: 'nothing_to_update' }, 400);
  vals.push(c.req.param('whId'), userId);
  await (c.env.DB as D1Database).prepare(`UPDATE webhooks SET ${updates.join(', ')} WHERE id = ? AND user_id = ?`).bind(...vals).run();
  const wh = await getWebhook(c.env.DB, c.req.param('whId'), userId);
  if (!wh) return c.json({ error: 'not_found' }, 404);
  return c.json(webhookShape(wh));
});
...
dev.post('/webhooks/:whId/toggle', async (c) => {
  ...
  const wh = await getWebhook(c.env.DB, c.req.param('whId'), userId);
  if (!wh) return c.json({ error: 'not_found' }, 404);
  await (c.env.DB as D1Database).prepare('UPDATE webhooks SET enabled = ? WHERE id = ? AND user_id = ?')
    .bind(wh.enabled ? 0 : 1, wh.id, userId).run();
  const rows = await (c.env.DB as D1Database).prepare(
    'SELECT id, description, url, secret_prefix, events, enabled, last_delivery_at, last_delivery_code, last_delivery_status, created_at FROM webhooks WHERE user_id = ? ORDER BY created_at DESC'
  ).bind(userId).all<any>();
  return c.json((rows.results ?? []).map(webhookShape));
...
dev.post('/webhooks/:whId/rotate-secret', async (c) => {
  ...
  const hash = await hashToken(raw);
  await (c.env.DB as D1Database).prepare('UPDATE webhooks SET secret_hash = ?, secret_prefix = ? WHERE id = ? AND user_id = ?')
    .bind(hash, prefix, wh.id, userId).run();
...
dev.post('/webhooks/:whId/test', async (c) => {
  ...
  try {
    ...
    const res = await fetch(wh.url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Webhook-Signature': sig, 'X-Webhook-Test': '1' },
      body: payload,
    });
    code = res.status;
  ...
  ).bind(deliveryId, wh.id, event, status, code, latencyMs, now).run();
  await (c.env.DB as D1Database).prepare(
    'UPDATE webhooks SET last_delivery_at = ?, last_delivery_code = ?, last_delivery_status = ? WHERE id = ?'
  ).bind(now, code, status, wh.id).run();

  return c.json({ delivery: { id: deliveryId, event, status, code, latencyMs, timestamp: now } });
});

dev.delete('/webhooks/:whId', async (c) => {
  const { userId } = c.get('user') as UserSession;
  await (c.env.DB as D1Database).prepare('DELETE FROM webhooks WHERE id = ? AND user_id = ?').bind(c.req.param('whId'), userId).run();
...
dev.get('/settings', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const row = await (c.env.DB as D1Database).prepare('SELECT * FROM dev_settings WHERE user_id = ?').bind(userId).first<any>();
...
dev.patch('/settings', async (c) => {
  ...
  const body = await c.req.json<Record<string, unknown>>();
  const colMap: Record<string, string> = {
    ipAllowlist: 'ip_allowlist', requireSigning: 'require_signing',
    sandboxMode: 'sandbox_mode', verboseLogging: 'verbose_logging', betaAccess: 'beta_access',
  };
  const current = (await (c.env.DB as D1Database).prepare('SELECT * FROM dev_settings WHERE user_id = ?').bind(userId).first<any>())
    ?? { ip_allowlist: '[]', require_signing: 1, sandbox_mode: 0, verbose_logging: 0, beta_access: 0 };

  const merged: Record<string, unknown> = { ...current };
  for (const [k, v] of Object.entries(body)) {
    const col = colMap[k];
    if (!col) continue;
    merged[col] = col === 'ip_allowlist' ? JSON.stringify(v) : (typeof v === 'boolean' ? (v ? 1 : 0) : v);
  }
  ...
  ).bind(userId, merged.ip_allowlist, merged.require_signing, merged.sandbox_mode, merged.verbose_logging, merged.beta_access, Date.now()).run();
...
async function getWebhook(db: D1Database, id: string, userId: string) {
  return db.prepare('SELECT * FROM webhooks WHERE id = ? AND user_id = ?').bind(id, userId).first<any>();
}

function deliveryShape(d: any) {
  return { id: d.id, event: d.event, status: d.status, code: d.code, latencyMs: d.latency_ms, timestamp: d.created_at };
}

// Relay groups
dev.get('/relay', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const rows = await (c.env.DB as D1Database).prepare('SELECT * FROM relay_groups WHERE user_id = ? ORDER BY created_at DESC').bind(userId).all<any>();
  return c.json((rows.results ?? []).map((r: any) => ({
    id: r.id, name: r.name, members: r.member_count, status: r.status, createdAt: r.created_at,
  })));
});
...
dev.post('/relay', async (c) => {
  ...
  if (!name?.trim()) return c.json({ error: 'name_required' }, 400);
  const id = uid();
  await (c.env.DB as D1Database).prepare('INSERT INTO relay_groups (id, user_id, name, status, member_count, created_at) VALUES (?, ?, ?, ?, 0, ?)')
    .bind(id, userId, name, 'online', Date.now()).run();
  return c.json({ id, name, members: 0, status: 'online' }, 201);
});

function webhookShape(w: any) {
  return {
    id: w.id, description: w.description, url: w.url, secretPrefix: w.secret_prefix,
    events: JSON.parse(w.events || '[]'), enabled: !!w.enabled,
    lastDeliveryAt: w.last_delivery_at, lastDeliveryCode: w.last_delivery_code,
    lastDeliveryStatus: w.last_delivery_status, createdAt: w.created_at,
  };
}

// ─── Stats ────────────────────────────────────────────────────────────────────
export const stats = new Hono<{ Bindings: any; Variables: { user: UserSession } }>();
stats.use('*', requireAuth);

stats.get('/summary', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const today = new Date().toISOString().slice(0, 10);
  const bots = await (c.env.DB as D1Database).prepare('SELECT id, name FROM bots WHERE user_id = ?').bind(userId).all<any>();
  const botList = bots.results ?? [];

  const rates = await Promise.all(botList.map(async (b: any) => {
    const row = await (c.env.DB as D1Database).prepare(
      'SELECT message_count FROM bot_analytics WHERE bot_id = ? AND date = ?'
    ).bind(b.id, today).first<{ message_count: number }>();
...
stats.get('/bots/:id/rate', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const bot = await (c.env.DB as D1Database).prepare('SELECT id FROM bots WHERE id = ? AND user_id = ?').bind(c.req.param('id'), userId).first<any>();
...
// ─── Files (R2) ───────────────────────────────────────────────────────────────
export const files = new Hono<{ Bindings: any; Variables: { user: UserSession } }>();
...
files.post('/upload', async (c) => {
  ...
  return c.json({ url, key, name: file.name, size: file.size, type: file.type }, 201);
});

files.get('/:key{.+}', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const key = c.req.param('key');
  if (!key.startsWith(`uploads/${userId}/`)) return c.json({ error: 'not_found' }, 404);
...
webhook.post('/:token', async (c) => {
  const limited = await checkBotLimits(c.env.KV as KVNamespace, { surface: 'inbound-webhook', ip: requestIp(c.req.raw), limit: 120, windowSec: 60 });
  ...
  if (result.stored && !result.muted && result.text && c.env.FCM_SERVER_KEY) {
    ...
    ).bind(result.botId).first<{ name: string; user_id: string; enable_notifications: number; notify_on_message: number }>();
    if (bot && bot.enable_notifications && bot.notify_on_message) {
      ...
      ).bind(bot.user_id).all<{ fcm_token: string }>();
      const tokens = (sessions.results ?? []).map(s => s.fcm_token).filter(Boolean) as string[];
      if (tokens.length) {
        const { sendFCMToTokens, botAlertPayload } = await import('../lib/fcm');
        await sendFCMToTokens(c.env.FCM_SERVER_KEY, tokens,
          botAlertPayload(bot.name, `New message: ${result.text.slice(0, 60)}`, result.botId));
      }
    }
  }

  return c.json({ ok: true, messageId: result.stored ? result.messageId : null });
});
