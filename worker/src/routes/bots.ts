...
import { getBotAnalyticsSummary, getBotAnalyticsDaily, refreshBotAnalyticsForToday } from '../lib/botAnalytics';
...
// are unchanged.
import { COMMAND_RE, COMMAND_ACTION_TYPES, normalizeCommandName, DEFAULT_COMMANDS } from '../lib/botCommands';
import { deliverAndRecordOutbound, storeOutboundMessage, updateConversation, updateBotUserActivity } from '../lib/botMessagePipeline';
import { checkBotLimits, requestIp } from '../lib/botRateLimit';
// PHASE 16 — Bot Webhook System.
import {
  checkWebhookUrl, generateWebhookSecret, hashToken as hashWebhookSecret,
  deliverBotWebhookOnce, deliverBotWebhookWithRetry, BotWebhookRow,
...
bots.use('*', async (c, next) => {
  ...
  const result = await checkBotLimits(c.env.KV as KVNamespace, {
    surface: 'bot-mutation', botId, ip: requestIp(c.req.raw), limit: 120, windowSec: 60,
  });
  if (!result.allowed) {
...
bots.post('/', async (c) => {
  ...
  const body = await c.req.json<{
    name: string; username: string; description?: string; category?: string;
    avatarColor?: string; welcomeMessage?: string; webhookUrl?: string; profileImage?: string;
    enableBot?: boolean; allowMessages?: boolean; allowFiles?: boolean;
    allowCommands?: boolean; enableNotifications?: boolean;
  }>();
  ...
  if (!USERNAME_RE.test(usernameClean)) {
    return c.json({ error: 'invalid_username', message: 'At least 4 characters, starting with a letter — letters, numbers, underscores only.' }, 400);
  }

  if (body.webhookUrl !== undefined && !isValidWebhookUrl(body.webhookUrl)) {
    return c.json({ error: 'invalid_webhook_url', message: 'Webhook URL must be a valid http(s) URL, or empty to omit it.' }, 400);
  }

  const db: D1Database = c.env.DB;
  const taken = await db.prepare('SELECT id FROM bots WHERE username = ?').bind(usernameClean).first();
  if (taken) return c.json({ error: 'username_taken', message: 'That bot username is taken.' }, 409);
  ...
  const colors = ['#2E4A3E','#3D6E8A','#8A5A3D','#6E3D8A','#3D8A6E','#8A3D5A','#4A3D2E','#2E3E4A'];
  ...
  await db.prepare(
    `INSERT INTO bots (id, user_id, name, username, description, category, avatar_color, profile_image_url, welcome_message,
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'offline', ?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, userId, body.name.trim(), usernameClean, body.description ?? '',
    body.category ?? 'Utilities', avatarColor, body.profileImage ?? null, body.welcomeMessage ?? '',
    tokenHash, tokenPrefix, body.webhookUrl ?? null,
    enabled ? 1 : 0, allowMessages ? 1 : 0, allowFiles ? 1 : 0, allowCommands ? 1 : 0, enableNotifications ? 1 : 0,
    now, now).run();
  ...
  for (const [i, def] of DEFAULT_COMMANDS.entries()) {
    ...
    ).bind(uid(), id, def.command, def.description, def.actionType, def.actionValue, now + i).run();
  }

  const bot = await db.prepare('SELECT * FROM bots WHERE id = ?').bind(id).first<any>();
...
bots.get('/:id', async (c) => {
  ...
  const [cmdRows, userRows, analyticsSummary] = await Promise.all([
    (c.env.DB as D1Database).prepare('SELECT * FROM bot_commands WHERE bot_id = ? ORDER BY created_at ASC').bind(bot.id).all<any>(),
    (c.env.DB as D1Database).prepare('SELECT * FROM bot_users WHERE bot_id = ? ORDER BY last_active_at DESC').bind(bot.id).all<any>(),
...
bots.patch('/:id', async (c) => {
  ...
  if (body.username !== undefined) {
    const usernameClean = String(body.username).trim().replace(/^@/, '').toLowerCase();
    if (!USERNAME_RE.test(usernameClean)) {
      return c.json({ error: 'invalid_username', message: 'At least 4 characters, starting with a letter — letters, numbers, underscores only.' }, 400);
    }
    if (usernameClean !== bot.username) {
      const taken = await db.prepare('SELECT id FROM bots WHERE username = ? AND id != ?').bind(usernameClean, bot.id).first();
      if (taken) return c.json({ error: 'username_taken', message: 'That bot username is taken.' }, 409);
    }
  ...
  if (body.webhookUrl !== undefined && !isValidWebhookUrl(body.webhookUrl)) {
    return c.json({ error: 'invalid_webhook_url', message: 'Webhook URL must be a valid http(s) URL, or empty to clear it.' }, 400);
  }
  ...
  for (const [k, v] of Object.entries(body)) {
    ...
    if (col) { updates.push(`${col} = ?`); vals.push(v); }
  }
  if (!updates.length) return c.json({ error: 'nothing_to_update' }, 400);

  updates.push('updated_at = ?');
  vals.push(Date.now(), bot.id);
  await db.prepare(`UPDATE bots SET ${updates.join(', ')} WHERE id = ?`).bind(...vals).run();

  const updated = await db.prepare('SELECT * FROM bots WHERE id = ?').bind(bot.id).first<any>();
...
bots.post('/:id/enable', async (c) => {
  ...
  await db.prepare('UPDATE bots SET enabled = 1, status = ?, updated_at = ? WHERE id = ?')
    .bind(nextStatus, Date.now(), bot.id).run();
...
bots.post('/:id/disable', async (c) => {
  ...
  const db = c.env.DB as D1Database;
  await db.prepare("UPDATE bots SET enabled = 0, status = 'disabled', updated_at = ? WHERE id = ?")
...
bots.post('/:id/rotate-token', async (c) => {
  ...
  await (c.env.DB as D1Database)
    .prepare('UPDATE bots SET token_hash = ?, token_prefix = ?, token_revoked = 0, updated_at = ? WHERE id = ?')
    .bind(hash, prefix, Date.now(), bot.id).run();
...
bots.delete('/:id', async (c) => {
  ...
  const bot = await getBot(c.env.DB, c.req.param('id'), userId);
  if (!bot) return c.json({ error: 'not_found' }, 404);
  await (c.env.DB as D1Database).prepare('DELETE FROM bots WHERE id = ?').bind(bot.id).run();
...
async function getBotWebhook(db: D1Database, botId: string): Promise<BotWebhookRow | null> {
  return db.prepare('SELECT * FROM bot_webhooks WHERE bot_id = ?').bind(botId).first<BotWebhookRow>();
}
...
const WEBHOOK_URL_ERROR_MESSAGE: Record<string, string> = {
  ...
  blocked_host: 'That host is not allowed for a webhook (local/private/link-local addresses are blocked).',
};
...
bots.post('/:id/webhook', async (c) => {
  ...
  if (existing) return c.json({ error: 'already_configured', message: 'This bot already has a webhook. Use PATCH to update it.' }, 409);

  const body = await c.req.json<{ url?: string }>();
  const checked = checkWebhookUrl(body.url);
  if (!checked.ok) return c.json({ error: `invalid_webhook_url:${checked.error}`, message: WEBHOOK_URL_ERROR_MESSAGE[checked.error] }, 400);
  ...
  await db.prepare(
    `INSERT INTO bot_webhooks (id, bot_id, url, secret_hash, secret_prefix, enabled, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, 1, ?, ?)`
  ).bind(id, bot.id, checked.url, secretHash, secretPrefix, now, now).run();
  await db.prepare('UPDATE bots SET webhook_url = ?, updated_at = ? WHERE id = ?').bind(checked.url, now, bot.id).run();
...
bots.patch('/:id/webhook', async (c) => {
  ...
  const body = await c.req.json<{ url?: string }>();
  if (body.url === undefined) return c.json({ error: 'nothing_to_update' }, 400);
  const checked = checkWebhookUrl(body.url);
  if (!checked.ok) return c.json({ error: `invalid_webhook_url:${checked.error}`, message: WEBHOOK_URL_ERROR_MESSAGE[checked.error] }, 400);

  const db = c.env.DB as D1Database;
  const now = Date.now();
  await db.prepare('UPDATE bot_webhooks SET url = ?, updated_at = ? WHERE id = ?').bind(checked.url, now, wh.id).run();
...
bots.post('/:id/webhook/enable', async (c) => {
  ...
  const db = c.env.DB as D1Database;
  const now = Date.now();
  await db.prepare('UPDATE bot_webhooks SET enabled = 1, updated_at = ? WHERE id = ?').bind(now, wh.id).run();
  await db.prepare('UPDATE bots SET webhook_url = ?, updated_at = ? WHERE id = ?').bind(wh.url, now, bot.id).run();
  const updated = await getBotWebhook(db, bot.id);
  return c.json(webhookShape(updated!));
});
...
bots.post('/:id/webhook/disable', async (c) => {
  ...
  const db = c.env.DB as D1Database;
  const now = Date.now();
  await db.prepare('UPDATE bot_webhooks SET enabled = 0, updated_at = ? WHERE id = ?').bind(now, wh.id).run();
  ...
  await db.prepare("UPDATE bots SET webhook_url = NULL, updated_at = ? WHERE id = ?").bind(now, bot.id).run();
  const updated = await getBotWebhook(db, bot.id);
  return c.json(webhookShape(updated!));
});
...
bots.post('/:id/webhook/rotate-secret', async (c) => {
  ...
  const secretHash = await hashWebhookSecret(secret);
  await (c.env.DB as D1Database)
    .prepare('UPDATE bot_webhooks SET secret_hash = ?, secret_prefix = ?, updated_at = ? WHERE id = ?')
    .bind(secretHash, secretPrefix, Date.now(), wh.id).run();
...
bots.post('/:id/webhook/test', async (c) => {
  ...
  const { result, eventId } = await deliverBotWebhookOnce(
    c.env.DB as D1Database, wh, wh.secret_hash, 'bot.webhook.test',
    { botId: bot.id, botName: bot.name, sentAt: Date.now() },
  );
  return c.json({ delivery: { eventId, ok: result.ok, code: result.code, latencyMs: result.latencyMs } });
});
...
bots.delete('/:id/webhook', async (c) => {
  ...
  if (!bot) return c.json({ error: 'not_found' }, 404);
  const db = c.env.DB as D1Database;
  await db.prepare('DELETE FROM bot_webhooks WHERE bot_id = ?').bind(bot.id).run();
  await db.prepare('UPDATE bots SET webhook_url = NULL, updated_at = ? WHERE id = ?').bind(Date.now(), bot.id).run();
  return c.body(null, 204);
});
...
async function getCommand(db: D1Database, botId: string, cmdId: string) {
  return db.prepare('SELECT * FROM bot_commands WHERE id = ? AND bot_id = ?').bind(cmdId, botId).first<any>();
}
...
bots.get('/:id/commands', async (c) => {
  ...
  const bot = await getBot(c.env.DB, c.req.param('id'), userId);
  if (!bot) return c.json({ error: 'not_found' }, 404);
  const rows = await (c.env.DB as D1Database).prepare('SELECT * FROM bot_commands WHERE bot_id = ? ORDER BY created_at ASC').bind(bot.id).all<any>();
...
bots.post('/:id/commands', async (c) => {
  ...
  const body = await c.req.json<{ command: string; description?: string; actionType?: string; actionValue?: string; enabled?: boolean }>();
  const commandClean = normalizeCommandName(body.command);
  if (!COMMAND_RE.test(commandClean)) {
    return c.json({ error: 'invalid_command', message: 'Lowercase letters, numbers, underscores — must start with a letter, up to 32 characters.' }, 400);
  }

  const actionType = body.actionType ?? 'No action';
  if (!COMMAND_ACTION_TYPES.has(actionType)) {
    return c.json({ error: 'invalid_action_type', message: `actionType must be one of: ${[...COMMAND_ACTION_TYPES].join(', ')}` }, 400);
  }

  const db = c.env.DB as D1Database;
  const dup = await db.prepare('SELECT id FROM bot_commands WHERE bot_id = ? AND command = ?').bind(bot.id, commandClean).first();
  if (dup) return c.json({ error: 'command_taken', message: `/${commandClean} already exists on this bot.` }, 409);

  const id = uid();
  const now = Date.now();
  await db.prepare(
    'INSERT INTO bot_commands (id, bot_id, command, description, action_type, action_value, enabled, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
  ).bind(id, bot.id, commandClean, body.description ?? '', actionType, body.actionValue ?? '', (body.enabled ?? true) ? 1 : 0, now).run();
...
bots.patch('/:id/commands/:cmdId', async (c) => {
  ...
  if (body.command !== undefined) {
    ...
    if (commandClean !== existing.command) {
      const dup = await db.prepare('SELECT id FROM bot_commands WHERE bot_id = ? AND command = ? AND id != ?')
        .bind(bot.id, commandClean, existing.id).first();
      if (dup) return c.json({ error: 'command_taken', message: `/${commandClean} already exists on this bot.` }, 409);
    }
    body.command = commandClean;
  }

  if (body.actionType !== undefined && !COMMAND_ACTION_TYPES.has(String(body.actionType))) {
    return c.json({ error: 'invalid_action_type', message: `actionType must be one of: ${[...COMMAND_ACTION_TYPES].join(', ')}` }, 400);
  }

  const allowed = ['command', 'description', 'action_type', 'action_value', 'enabled'];
  const updates: string[] = [];
  const vals: unknown[] = [];
  for (const [k, v] of Object.entries(body)) {
    const col = camelToSnake(k);
    if (allowed.includes(col)) { updates.push(`${col} = ?`); vals.push(col === 'enabled' ? (v ? 1 : 0) : v); }
  }
  if (!updates.length) return c.json({ error: 'nothing_to_update' }, 400);
  vals.push(existing.id, bot.id);
  await db.prepare(`UPDATE bot_commands SET ${updates.join(', ')} WHERE id = ? AND bot_id = ?`).bind(...vals).run();
...
bots.post('/:id/commands/:cmdId/enable', async (c) => {
  ...
  const existing = await getCommand(db, bot.id, c.req.param('cmdId'));
  if (!existing) return c.json({ error: 'not_found' }, 404);
  await db.prepare('UPDATE bot_commands SET enabled = 1 WHERE id = ? AND bot_id = ?').bind(existing.id, bot.id).run();
  const cmd = await getCommand(db, bot.id, existing.id);
  return c.json(commandShape(cmd!));
});
...
bots.post('/:id/commands/:cmdId/disable', async (c) => {
  ...
  const existing = await getCommand(db, bot.id, c.req.param('cmdId'));
  if (!existing) return c.json({ error: 'not_found' }, 404);
  await db.prepare('UPDATE bot_commands SET enabled = 0 WHERE id = ? AND bot_id = ?').bind(existing.id, bot.id).run();
  const cmd = await getCommand(db, bot.id, existing.id);
  return c.json(commandShape(cmd!));
});
...
bots.delete('/:id/commands/:cmdId', async (c) => {
  ...
  const existing = await getCommand(db, bot.id, c.req.param('cmdId'));
  if (!existing) return c.json({ error: 'not_found' }, 404);
  await db.prepare('DELETE FROM bot_commands WHERE id = ? AND bot_id = ?').bind(existing.id, bot.id).run();
  return c.body(null, 204);
});
...
async function getBotUser(db: D1Database, botId: string, botUserId: string) {
  return db.prepare('SELECT * FROM bot_users WHERE id = ? AND bot_id = ?').bind(botUserId, botId).first<any>();
}
...
bots.get('/:id/users', async (c) => {
  ...
  const bot = await getBot(c.env.DB, c.req.param('id'), userId);
  if (!bot) return c.json({ error: 'not_found' }, 404);
  const rows = await (c.env.DB as D1Database).prepare('SELECT * FROM bot_users WHERE bot_id = ? ORDER BY last_active_at DESC').bind(bot.id).all<any>();
...
bots.patch('/:id/users/:userId', async (c) => {
  ...
  const updates: string[] = [];
  const vals: unknown[] = [];
  if (body.blocked !== undefined) { updates.push('blocked = ?'); vals.push(body.blocked ? 1 : 0); }
  if (body.muted !== undefined) { updates.push('muted = ?'); vals.push(body.muted ? 1 : 0); }
  if (!updates.length) return c.json({ error: 'nothing_to_update' }, 400);
  vals.push(c.req.param('userId'), bot.id);
  await (c.env.DB as D1Database).prepare(`UPDATE bot_users SET ${updates.join(', ')} WHERE id = ? AND bot_id = ?`).bind(...vals).run();
...
async function setBotUserFlag(c: any, column: 'blocked' | 'muted', value: boolean) {
  const { userId } = c.get('user') as UserSession;
  const bot = await getBot(c.env.DB, c.req.param('id'), userId);
  if (!bot) return c.json({ error: 'not_found' }, 404);
  const db = c.env.DB as D1Database;
  ...
  if (!existing) return c.json({ error: 'not_found' }, 404);
  await db.prepare(`UPDATE bot_users SET ${column} = ? WHERE id = ? AND bot_id = ?`)
    .bind(value ? 1 : 0, existing.id, bot.id).run();
  const u = await getBotUser(db, bot.id, existing.id);
  return c.json(botUserShape(u!));
}

bots.post('/:id/users/:userId/block', (c) => setBotUserFlag(c, 'blocked', true));
bots.post('/:id/users/:userId/unblock', (c) => setBotUserFlag(c, 'blocked', false));
bots.post('/:id/users/:userId/mute', (c) => setBotUserFlag(c, 'muted', true));
bots.post('/:id/users/:userId/unmute', (c) => setBotUserFlag(c, 'muted', false));
...
bots.get('/:id/analytics/daily', async (c) => {
  ...
  const daily = await getBotAnalyticsDaily(c.env.DB as D1Database, bot.id, days);
  return c.json(daily);
});
...
function botMessageShape(m: any) {
  return {
    ...
    dir: m.direction,
    text: m.text,
    attachment: m.attachment_url ? { url: m.attachment_url, kind: m.attachment_type } : null,
...
bots.get('/:id/users/:userId/messages', async (c) => {
  ...
  const limit = Math.max(1, Math.min(Number(c.req.query('limit') ?? 100) || 100, 200));
  const rows = await (c.env.DB as D1Database).prepare(
    'SELECT * FROM bot_messages WHERE bot_id = ? AND bot_user_id = ? ORDER BY created_at ASC LIMIT ?'
  ).bind(bot.id, botUser.id, limit).all<any>();
  return c.json((rows.results ?? []).map(botMessageShape));
});
...
bots.post('/:id/users/:userId/messages', async (c) => {
  ...
  const body = await c.req.json<{ text?: string; attachmentUrl?: string; attachmentType?: string }>();
  if (!body.text && !body.attachmentUrl) return c.json({ error: 'empty_message' }, 400);

  const db = c.env.DB as D1Database;
  const attachment = body.attachmentUrl ? { url: body.attachmentUrl, type: body.attachmentType ?? null } : null;
  ...
  const outbound = await storeOutboundMessage(db, bot.id, botUser.id, body.text ?? null, 'admin', attachment);
  await updateConversation(db, bot.id, botUser.id, body.text ?? null, attachment, outbound.createdAt);
  await db.prepare('UPDATE bot_conversations SET last_message_id=?, updated_at=? WHERE bot_id=? AND bot_user_id=?').bind(outbound.id,outbound.createdAt,bot.id,botUser.id).run();
  ...
  const delivery = await deliverAndRecordOutbound(
    db, bot as any, botUser.id,
    { id: outbound.id, text: body.text ?? null, createdAt: outbound.createdAt },
    null,
  );

  return c.json({
    id: outbound.id, botId: bot.id, userId: botUser.id, dir: 'out',
    text: body.text ?? null, attachmentUrl: body.attachmentUrl ?? null,
    attachmentType: body.attachmentType ?? null, createdAt: outbound.createdAt,
    status: delivery.status, deliveryCode: delivery.code,
  }, 201);
});
...
bots.get('/:id/conversations', async (c) => {
  ...
  return c.json((rows.results ?? []).map((r:any) => ({ id:r.id, botId:r.bot_id, botUserId:r.bot_user_id, name:r.name, username:r.username, unreadCount:r.unread_count, lastMessageAt:r.last_message_at, lastMessage:r.last_message_id ? { id:r.last_message_id, text:r.last_message_text, direction:r.last_message_direction, status:r.last_message_status } : null, blocked:!!r.blocked, muted:!!r.muted })));
});
...
bots.get('/:id/conversations/:userId/messages', async (c) => {
  ...
  if (!botUser) return c.json({ error: 'not_found' }, 404);
  const db = c.env.DB as D1Database;
  await db.prepare('UPDATE bot_conversations SET unread_count=0, updated_at=? WHERE bot_id=? AND bot_user_id=?').bind(Date.now(),bot.id,botUser.id).run();
  const rows = await db.prepare('SELECT * FROM bot_messages WHERE bot_id=? AND bot_user_id=? ORDER BY created_at ASC LIMIT 200').bind(bot.id,botUser.id).all<any>();
  return c.json((rows.results ?? []).map(botMessageShape));
});

// PHASE 19 - private R2 bot files. Object keys are generated server-side and never returned.
const BOT_FILE_TYPES = new Set(['image/jpeg','image/png','image/webp','application/pdf','text/plain','audio/mpeg','audio/mp4','video/mp4']);
...
bots.post('/:id/files', async (c) => {
  ...
  const limited = await checkBotLimits(c.env.KV,{surface:'file',botId:bot.id,ip:requestIp(c.req.raw),limit:20,windowSec:60});
  if (!limited.allowed) return c.json({error:'rate_limited',retryAfter:limited.retryAfter},429,{ 'Retry-After':String(limited.retryAfter) });
  const form = await c.req.formData(); const file = form.get('file');
  if (!file || typeof file === 'string' || typeof (file as Blob).arrayBuffer !== 'function') return c.json({error:'file_required'},400);
  const uploadFile = file as File;
  if (!BOT_FILE_TYPES.has(uploadFile.type)) return c.json({error:'mime_not_allowed'},415);
  if (uploadFile.size<=0 || uploadFile.size>BOT_FILE_MAX) return c.json({error:'file_too_large',maxBytes:BOT_FILE_MAX},413);
  const botUserId = String(form.get('botUserId') ?? '') || null;
  if (botUserId && !(await getBotUser(c.env.DB,bot.id,botUserId))) return c.json({error:'not_found'},404);
  const id=uid(), now=Date.now(), ext=(uploadFile.name.split('.').pop()||'bin').replace(/[^a-zA-Z0-9]/g,'').slice(0,8)||'bin';
  const key=`bots/${bot.id}/users/${botUserId ?? 'owner'}/${id}.${ext}`;
  await c.env.BUCKET.put(key,await uploadFile.arrayBuffer(),{httpMetadata:{contentType:uploadFile.type},customMetadata:{fileId:id,botId:bot.id}});
  await (c.env.DB as D1Database).prepare('INSERT INTO bot_files (id,bot_id,bot_user_id,object_key,original_name,mime_type,byte_size,created_at) VALUES (?,?,?,?,?,?,?,?)').bind(id,bot.id,botUserId,uploadFile.name.slice(0,255),uploadFile.type,uploadFile.size,now).run();
  return c.json({id,botId:bot.id,botUserId,name:uploadFile.name.slice(0,255),mimeType:uploadFile.type,size:uploadFile.size,createdAt:now,url:`/bots/${bot.id}/files/${id}`},201);
});

bots.get('/:id/files', async (c) => {
 const {userId}=c.get('user') as UserSession; const bot=await getBot(c.env.DB,c.req.param('id'),userId); if(!bot)return c.json({error:'not_found'},404);
 const rows=await (c.env.DB as D1Database).prepare('SELECT id,bot_id,bot_user_id,original_name,mime_type,byte_size,created_at FROM bot_files WHERE bot_id=? ORDER BY created_at DESC LIMIT 100').bind(bot.id).all<any>();
 return c.json((rows.results??[]).map((r:any)=>({id:r.id,botId:r.bot_id,botUserId:r.bot_user_id,name:r.original_name,mimeType:r.mime_type,size:r.byte_size,createdAt:r.created_at,url:`/bots/${bot.id}/files/${r.id}`})));
});

bots.get('/:id/files/:fileId', async (c) => {
 const {userId}=c.get('user') as UserSession; const bot=await getBot(c.env.DB,c.req.param('id'),userId); if(!bot)return c.json({error:'not_found'},404);
 const meta=await (c.env.DB as D1Database).prepare('SELECT * FROM bot_files WHERE id=? AND bot_id=?').bind(c.req.param('fileId'),bot.id).first<any>(); if(!meta)return c.json({error:'not_found'},404);
 const obj=await c.env.BUCKET.get(meta.object_key); if(!obj)return c.json({error:'not_found'},404); const h=new Headers({'Content-Type':meta.mime_type,'Content-Length':String(meta.byte_size),'Content-Disposition':`inline; filename="${encodeURIComponent(meta.original_name)}"`}); h.set('ETag',obj.httpEtag); return new Response(obj.body,{headers:h});
});

bots.delete('/:id/files/:fileId', async (c) => {
 const {userId}=c.get('user') as UserSession; const bot=await getBot(c.env.DB,c.req.param('id'),userId); if(!bot)return c.json({error:'not_found'},404);
 const db=c.env.DB as D1Database; const meta=await db.prepare('SELECT * FROM bot_files WHERE id=? AND bot_id=?').bind(c.req.param('fileId'),bot.id).first<any>(); if(!meta)return c.json({error:'not_found'},404);
 await c.env.BUCKET.delete(meta.object_key); await db.prepare('DELETE FROM bot_files WHERE id=? AND bot_id=?').bind(meta.id,bot.id).run(); return c.body(null,204);
});

// ── Helpers ───────────────────────────────────────────────────────────────────
async function getBot(db: D1Database, botId: string, userId: string) {
  return db.prepare('SELECT * FROM bots WHERE id = ? AND user_id = ?').bind(botId, userId).first<any>();
}
...
function botUserShape(u: any) {
  return {
    ...
    joinedAt: u.joined_at, lastActiveAt: u.last_active_at,
    messageCount: u.message_count, blocked: !!u.blocked, muted: !!u.muted,
  };
}
