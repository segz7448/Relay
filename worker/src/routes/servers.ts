...
import { addChannelMember, canReadChannel, listChannelMembers, removeChannelMember } from '../lib/channelMembers';
...
servers.get('/', async (c) => {
  ...
  const full = await Promise.all(serverList.map(s => enrichServer(c.env.DB, s, userId)));
  return c.json(full);
});
...
servers.post('/', async (c) => {
  ...
  const body = await c.req.json<{ name: string; description?: string; iconColor?: string; privacy?: string }>();
  ...
  await db.prepare(
    `INSERT INTO servers (id, owner_id, name, description, icon_color, privacy, invite_code, last_activity_at, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, userId, body.name.trim(), body.description ?? '',
    body.iconColor ?? '#3D6E8A', body.privacy ?? 'private', inviteCode, now, now, now).run();

  // Default category
  await db.prepare('INSERT INTO server_categories (id, server_id, name, position) VALUES (?, ?, ?, 0)')
    .bind(generalCatId, id, 'General').run();

  // Default general channel
  await db.prepare(
    'INSERT INTO channels (id, server_id, category_id, name, type, created_at) VALUES (?, ?, ?, ?, ?, ?)'
  ).bind(generalChId, id, generalCatId, 'general', 'general', now).run();

  // Owner as member
  await db.prepare(
    'INSERT INTO server_members (id, server_id, user_id, name, username, role, online, joined_at, permissions) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)'
  ).bind(memberId, id, userId, userName, username, 'owner', now, JSON.stringify({ administrator: true })).run();

  const server = await db.prepare('SELECT * FROM servers WHERE id = ?').bind(id).first<any>();
...
servers.patch('/:id', async (c) => {
  ...
  const allowed = ['name','description','icon_color','privacy','muted','unread_count'];
  const updates: string[] = [];
  const vals: unknown[] = [];
  for (const [k, v] of Object.entries(body)) {
    const col = camelToSnake(k);
    if (allowed.includes(col)) { updates.push(`${col} = ?`); vals.push(typeof v === 'boolean' ? (v ? 1 : 0) : v); }
  }
  if (!updates.length) return c.json({ error: 'nothing_to_update' }, 400);
  updates.push('updated_at = ?');
  vals.push(Date.now(), server.id);
  await (c.env.DB as D1Database).prepare(`UPDATE servers SET ${updates.join(', ')} WHERE id = ?`).bind(...vals).run();
  const updated = await (c.env.DB as D1Database).prepare('SELECT * FROM servers WHERE id = ?').bind(server.id).first<any>();
...
servers.delete('/:id', async (c) => {
  const { userId } = c.get('user') as UserSession;
  const server = await (c.env.DB as D1Database).prepare('SELECT * FROM servers WHERE id = ? AND owner_id = ?').bind(c.req.param('id'), userId).first<any>();
  if (!server) return c.json({ error: 'not_found' }, 404);
  await (c.env.DB as D1Database).prepare('DELETE FROM servers WHERE id = ?').bind(server.id).run();
...
servers.post('/:id/channels', async (c) => {
  ...
  const body = await c.req.json<{
    name: string; type?: string; topic?: string; categoryId?: string;
    postAccess?: string; private?: boolean;
  }>();
  if (!body.name?.trim()) return c.json({ error: 'name_required' }, 400);
  ...
  ).bind(id, server.id, body.categoryId ?? null, body.name.toLowerCase().replace(/\s+/g, '-'),
    body.type ?? 'text', body.topic ?? '', body.private ? 1 : 0, body.postAccess ?? 'everyone', Date.now()).run();
  ...
  const ch = await (c.env.DB as D1Database).prepare('SELECT * FROM channels WHERE id = ?').bind(id).first<any>();
...
servers.patch('/:id/channels/:chId', async (c) => {
  ...
  if (!server) return c.json({ error: 'not_found' }, 404);
  const body = await c.req.json<Record<string, unknown>>();
  const allowed = ['name','topic','type','post_access','notifications','private'];
  const updates: string[] = [];
  const vals: unknown[] = [];
  for (const [k, v] of Object.entries(body)) {
    const col = camelToSnake(k);
  ...
  await (c.env.DB as D1Database).prepare(`UPDATE channels SET ${updates.join(', ')} WHERE id = ? AND server_id = ?`).bind(...vals).run();
  ...
  const ch = await (c.env.DB as D1Database).prepare('SELECT * FROM channels WHERE id = ?').bind(c.req.param('chId')).first<any>();
...
servers.delete('/:id/channels/:chId', async (c) => {
  ...
  const server = await getServer(c.env.DB, c.req.param('id'), userId);
  if (!server) return c.json({ error: 'not_found' }, 404);
  await (c.env.DB as D1Database).prepare('DELETE FROM channels WHERE id = ? AND server_id = ?').bind(c.req.param('chId'), server.id).run();
  return c.body(null, 204);
});
...
servers.get('/:id/channels/:chId/messages', async (c) => {
  ...
  const channel = await getChannelForUser(c.env.DB, server.id, c.req.param('chId'), userId);
  ...
  ).bind(c.req.param('chId'), limit).all<any>();
...
servers.post('/:id/channels/:chId/messages', async (c) => {
  ...
  const channel = await getChannelForUser(c.env.DB, server.id, c.req.param('chId'), userId);
  if (!channel) return c.json({ error: 'not_found' }, 404);
  const body = await c.req.json<{ text?: string; attachmentUrl?: string; replyToId?: string }>();
  if (!body.text && !body.attachmentUrl) return c.json({ error: 'content_required' }, 400);
  ...
  ).bind(id, c.req.param('chId'), userId, name, body.text ?? null, 'text', body.attachmentUrl ?? null, '{}', body.replyToId ?? null, now).run();

  await (c.env.DB as D1Database).prepare('UPDATE servers SET last_activity_at = ? WHERE id = ?').bind(now, server.id).run();
  const msg = await (c.env.DB as D1Database).prepare('SELECT * FROM channel_messages WHERE id = ?').bind(id).first<any>();
...
servers.get('/:id/members', async (c) => {
  ...
  const server = await getServer(c.env.DB, c.req.param('id'), userId);
  if (!server) return c.json({ error: 'not_found' }, 404);
  const rows = await (c.env.DB as D1Database).prepare('SELECT * FROM server_members WHERE server_id = ? ORDER BY role DESC, joined_at ASC').bind(server.id).all<any>();
...
servers.patch('/:id/members/:memberId', async (c) => {
  ...
  const server = await getServer(c.env.DB, c.req.param('id'), userId);
  if (!server) return c.json({ error: 'not_found' }, 404);
  const body = await c.req.json<{ role?: string; permissions?: Record<string, boolean> }>();
  const updates: string[] = [];
  const vals: unknown[] = [];
  if (body.role) { updates.push('role = ?'); vals.push(body.role); }
  if (body.permissions) { updates.push('permissions = ?'); vals.push(JSON.stringify(body.permissions)); }
  if (!updates.length) return c.json({ error: 'nothing_to_update' }, 400);
  vals.push(c.req.param('memberId'), server.id);
  await (c.env.DB as D1Database).prepare(`UPDATE server_members SET ${updates.join(', ')} WHERE id = ? AND server_id = ?`).bind(...vals).run();
...
servers.delete('/:id/members/:memberId', async (c) => {
  ...
  const server = await getServer(c.env.DB, c.req.param('id'), userId);
  if (!server) return c.json({ error: 'not_found' }, 404);
  await (c.env.DB as D1Database).prepare('DELETE FROM server_members WHERE id = ? AND server_id = ?').bind(c.req.param('memberId'), server.id).run();
  return c.body(null, 204);
});
...
servers.get('/:id/channels/:chId/members', async (c) => {
  ...
  return c.json(members.map((m) => ({ userId: m.user_id, addedAt: m.added_at })));
});
...
servers.post('/:id/channels/:chId/members', async (c) => {
  ...
  const targetIsServerMember = await (c.env.DB as D1Database).prepare(
    'SELECT 1 FROM server_members WHERE server_id = ? AND user_id = ?'
  ).bind(server.id, targetUserId).first();
  if (!targetIsServerMember) return c.json({ error: 'not_a_server_member' }, 400);
...
async function getServer(db: D1Database, serverId: string, userId: string) {
  ...
  ).bind(userId, serverId).first<any>();
}
...
async function getChannelForUser(db: D1Database, serverId: string, channelId: string, userId: string) {
  const channel = await db.prepare('SELECT * FROM channels WHERE id = ? AND server_id = ?')
    .bind(channelId, serverId).first<any>();
...
async function enrichServer(db: D1Database, server: any, userId: string) {
  const [cats, members] = await Promise.all([
    ...
    db.prepare('SELECT * FROM server_members WHERE server_id = ? ORDER BY role DESC').bind(server.id).all<any>(),
  ]);

  const categoryList = cats.results ?? [];
  const channelRows = await db.prepare('SELECT * FROM channels WHERE server_id = ? ORDER BY created_at ASC').bind(server.id).all<any>();
  const allChannels = channelRows.results ?? [];

  const categories = categoryList.map((cat: any) => ({
    id: cat.id, name: cat.name,
    channels: allChannels.filter((ch: any) => ch.category_id === cat.id).map(chShape),
  }));

  const uncategorized = allChannels.filter((ch: any) => !ch.category_id).map(chShape);

  const member = (members.results ?? []).find((m: any) => m.user_id === userId);

  return {
    id: server.id, name: server.name, description: server.description,
    icon: null, iconColor: server.icon_color, privacy: server.privacy,
    inviteCode: server.invite_code, ownerId: server.owner_id,
    createdAt: server.created_at, lastActivityAt: server.last_activity_at,
    unreadCount: server.unread_count, muted: !!server.muted,
...
function chShape(ch: any) {
  return {
    id: ch.id, name: ch.name, type: ch.type, topic: ch.topic,
    unread: ch.unread_count, private: !!ch.private,
    postAccess: ch.post_access, notifications: ch.notifications,
  };
}

function memberShape(m: any) {
  return {
    id: m.id, userId: m.user_id, name: m.name, username: m.username,
    avatarColor: m.avatar_color, role: m.role, online: !!m.online,
    joinedAt: m.joined_at, permissions: JSON.parse(m.permissions || '{}'),
  };
}
