...
async function createAssertion(account: FirebaseServiceAccount, nowSeconds: number): Promise<string> {
...
async function getAccessToken(account: FirebaseServiceAccount): Promise<string> {
  const now = Date.now();
  if (cachedAccessToken?.owner === account.client_email && cachedAccessToken.expiresAt > now + 60_000) {
  ...
  if (!result.access_token) throw new Error('FCM OAuth token exchange returned no access token');
...
export async function sendFCMToToken(
  ...
  try {
    ...
    if (!response.ok) {
      return { success: false, error: `FCM send failed with HTTP ${response.status}` };
    }
...
export async function sendFCMToTokens(
  ...
  await Promise.allSettled(tokens.map(token => sendFCMToToken(serviceAccountJson, token, payload)));
}
