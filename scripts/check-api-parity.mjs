...
for (const { prefix, file, routerIdent } of mounts) {
  ...
  const owner = ["privacy", "dev", "stats", "files", "webhook"].includes(
    routerIdent,
  )
    ? routerIdent
    : "\\w+";
  const re = new RegExp(
    `${owner}\\.(get|post|patch|put|delete)\\([\"'](/[^\"']*)[\"']`,
    "g",
  );
...
function extractCalls(src) {
  ...
  while (i < src.length) {
    ...
    if (upIdx !== -1 && (reqIdx === -1 || upIdx < reqIdx)) {
      idx = upIdx;
      fn = "upload";
    } else {
      idx = reqIdx;
