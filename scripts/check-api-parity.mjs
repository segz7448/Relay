...
// ── Reverse audit (Phase 16-17 follow-up): Worker routes with no ─────
// frontend caller. Every backend route must either be reachable from the
// app or be explicitly documented here as intentionally non-UI, so an
// "implemented but never wired" endpoint (the Phase 16 webhook surface
// was exactly that) fails this check instead of shipping silently.
const clientNorm = new Set(clientCalls.map((c) => `${c.method} ${norm(c.path)}`));

// normalized "METHOD /path" → why the app intentionally never calls it.
// Every entry here is a deliberate, documented decision — adding one
// requires a reason a reviewer would accept.
const NON_UI_ROUTES = new Map([
  ['POST /webhook/{}', 'Public inbound receiver (Phase 8); callers are external sender platforms authenticating by bot token in the URL, not this app.'],
  ['GET /bot-runtime/me', "Bot-token runtime identity endpoint (Phase 9); called by external bot code with the bot's own token."],
  ['GET /bot-runtime/commands', 'Bot-token runtime command fetch (Phase 9/15); called by external bot code.'],
  ['GET /bots/{}/files/{}', 'Bot file bytes are streamed via utils/attachments.js (direct URL + Authorization header / web blob fetch), not api.js.'],
  ['GET /files/{}{.+}', 'General file bytes are streamed via utils/attachments.js (direct URL + Authorization header / web blob fetch), not api.js.'],
  ['GET /servers/{}/channels/{}/members', 'Private-channel membership management API (notes/06); enforced server-side, managed outside the app UI.'],
  ['POST /servers/{}/channels/{}/members', 'Private-channel membership management API (notes/06); enforced server-side, managed outside the app UI.'],
  ['DELETE /servers/{}/channels/{}/members/{}', 'Private-channel membership management API (notes/06); enforced server-side, managed outside the app UI.'],
  ['GET /dev/relay', 'Legacy developer-platform relay-group listing (worker/src/routes/other.ts); no app surface consumes relay groups.'],
  ['POST /dev/relay', 'Legacy developer-platform relay-group creation (worker/src/routes/other.ts); no app surface consumes relay groups.'],
]);

const uncovered = [...workerNorm].filter((r) => !clientNorm.has(r));
const undocumented = uncovered.filter((r) => !NON_UI_ROUTES.has(r));
if (uncovered.length) {
  console.log('\nWorker routes without an api.js caller (allowlisted as intentionally non-UI):');
  for (const r of uncovered.filter((r) => NON_UI_ROUTES.has(r)).sort()) console.log(`  ${r} — ${NON_UI_ROUTES.get(r)}`);
}
if (undocumented.length) {
  console.error('\nFAIL — Worker routes with no frontend caller and no documented reason:');
  for (const r of undocumented.sort()) console.error(`  ${r}`);
  process.exit(1);
}
console.log('OK — every Worker route is either called by the frontend or documented as intentionally non-UI.');
